...
class Test:
    ...
    def test_sign_up(self):
        try:
            with UserFacade() as user_facade:
                last_user_id = user_facade.sign_up("Efrat", "Gosh", "efgefg@outlook.com", "1234")
    ...
    def test_update_vacation(self):
        try:
            with VacationFacade() as vacation_facade:
                response = vacation_facade.update_vacation(13, 5, "Explore the historic city of Jerusalem, where ancient history and modern life intertwine. Visit the Old City, home to iconic landmarks such as the Western Wall, the Church of the Holy Sepulchre, and the bustling markets.", "2023-10-01", "2023-10-1", "6000", "jerusalem_haven_jpg")
                print(response)
        except Exception as err:
            print(err)
